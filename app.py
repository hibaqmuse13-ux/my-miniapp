...
from gTTS import gTTS  # <--- Maktabadda Codadka (Voice)
import requests
...
def process_hourly_profits():
  ...
  for inv in active_invs:
    ...
    if hours_passed >= TOTAL_HOURS:
      ...
      db.execute(
          'UPDATE users SET active_deposit = active_deposit - ? WHERE telegram_id = ?',
          (inv['amount'], user_id),
      )
      create_notification(
          user_id,
...
# ============================================================
# HELPER FUNCTIONS & VOICE NOTIFICATIONS
# ============================================================
...
def send_telegram_voice(chat_id, text):
  """Wuxuu qoraalka u beddelaa cod (Voice Note) wuxuuna u diraa Telegram-ka"""
  try:
...
def get_user(user_id):
  db = get_db()
  user = db.execute(
      'SELECT * FROM users WHERE telegram_id = ?', (str(user_id),)
  ).fetchone()
...
def verify_user():
  ...
  db = get_db()
  db.execute(
      'UPDATE users SET username = ?, last_login = ? WHERE telegram_id = ?',
...
def request_deposit():
  ...
  if not txid and not screenshot:
    return jsonify({
        'status': 'error',
        'message': '⚠️ Please enter TXID or upload a screenshot!',
    })

  db = get_db()
  # Anti-spam check for pending deposit
  pending_dep = db.execute(
      "SELECT * FROM transactions WHERE user_id = ? AND type = 'DEPOSIT' AND"
      " status = 'PENDING'",
      (str(user_id),),
  ).fetchone()
  if pending_dep:
    ...
    return jsonify({
        'status': 'error',
        'message': (
            '⚠️ Please wait for your pending deposit request to be processed'
  ...
  cursor = db.cursor()
  cursor.execute(
      'INSERT INTO transactions (user_id, type, amount, status, txid, network,'
      ' description) VALUES (?, ?, ?, ?, ?, ?, ?)',
      (
          user_id,
          'DEPOSIT',
          amount,
          'PENDING',
          txid if txid else 'SCREENSHOT_UPLOADED',
          network,
  ...
  keyboard = {
      'inline_keyboard': [[
          {
              'text': '✅ Approve',
              'callback_data': f'approve_dep_{tx_id}',
          },
          {
              'text': '❌ Reject',
              'callback_data': f'reject_dep_{tx_id}',
          },
      ]]
  }
  ...
  if screenshot:
    ...
    files = {
        'photo': (screenshot.filename, screenshot.read(), screenshot.content_type)
    }
    payload = {
        'chat_id': ADMIN_ID,
        'caption': admin_msg,
  ...
  # Dirista codka ogeysiinta Admin-ka
  send_telegram_voice(
      ADMIN_ID, f'New deposit request of {amount} dollars received.'
  )

  return jsonify({
      'status': 'success',
      'message': (
          '✅ Deposit request submitted successfully! Pending approval.'
      ),
  })
...
def invest():
  ...
  db = get_db()
  db.execute(
      'UPDATE users SET balance = balance - ?, active_deposit = active_deposit +'
      ' ? WHERE telegram_id = ?',
  ...
  create_notification(
      user_id,
      '🚀 Investment Started',
      f'Successfully invested ${amount} USDT!',
  )
  return jsonify({
      'status': 'success',
      'message': f'🎉 Successfully invested ${amount} USDT!',
  })
...
def request_withdrawal():
  ...
  pending_with = db.execute(
      "SELECT * FROM transactions WHERE user_id = ? AND type = 'WITHDRAWAL' AND"
      " status = 'PENDING'",
      (user_id,),
  ).fetchone()
  if pending_with:
    ...
    return jsonify({
        'status': 'error',
        'message': (
            '⚠️ Please wait for your pending withdrawal request to be processed'
            ' before submitting a new one.'
        ),
    })

  active_inv = db.execute(
      "SELECT * FROM investments WHERE user_id = ? AND status = 'ACTIVE'",
      (user_id,),
  ).fetchone()
  ...
  if active_inv:
    start_time = datetime.strptime(
        active_inv['start_date'], '%Y-%m-%d %H:%M:%S'
    )
  ...
  keyboard = {
      'inline_keyboard': [[
          {
              'text': '✅ Approve',
              'callback_data': f'approve_with_{tx_id}',
          },
          {
              'text': '❌ Reject',
              'callback_data': f'reject_with_{tx_id}',
          },
      ]]
  }
  admin_msg = (
  ...
  # Dirista codka ogeysiinta Admin-ka
  send_telegram_voice(
      ADMIN_ID, f'New withdrawal request of {amount} dollars received.'
  )
...
def send_support():
  ...
  subject = (
      data.get('subject')
      or data.get('subject_field')
      or 'General Inquiry'
  )
  message = (
      data.get('message')
      or data.get('message_detail')
      or data.get('text')
      or ''
  )
  ...
  pending_ticket = db.execute(
      "SELECT * FROM support_tickets WHERE user_id = ? AND status = 'PENDING'",
      (user_id,),
  ).fetchone()
  if pending_ticket:
    db.close()
    return jsonify({
        'status': 'error',
        'message': (
            '⚠️ You already have a pending support ticket. Please wait for a'
            ' reply before submitting a new one.'
        ),
    })
  ...
  keyboard = {
      'inline_keyboard': [[{
          ...
          'callback_data': f'reply_ticket_{user_id}',
      }]]
  }

  admin_msg = (
      f'🛠️ <b>NEW SUPPORT TICKET</b>\n\nUser: {username}\nID:'
      f' <code>{user_id}</code>\nSubject: <b>{subject}</b>\nStatus:'
      f' PENDING\n\nMessage:\n<i>{message}</i>'
  )
  send_telegram(ADMIN_ID, admin_msg, keyboard)
  ...
  # Dirista codka ogeysiinta Admin-ka
  send_telegram_voice(ADMIN_ID, 'New support ticket received.')

  return jsonify({
      'status': 'success',
      'message': (
          '✅ Support message sent successfully! Admin will contact you soon.'
      ),
  })
...
def get_user_tickets(user_id):
  ...
  cursor = db.cursor()
  cursor.execute(
      'SELECT id, subject, message, status, admin_reply, date FROM'
      ' support_tickets WHERE user_id = ? ORDER BY id DESC',
      (str(user_id),),
  )
...
def admin_get_users():
  db = get_db()
  users = db.execute(
      'SELECT telegram_id, username, balance, active_deposit, referral_code,'
...
def webhook():
  ...
  if update and 'message' in update:
    ...
    if chat_id == str(ADMIN_ID) and chat_id in admin_waiting_reply:
      ...
      db = get_db()
      ticket = db.execute(
          "SELECT * FROM support_tickets WHERE user_id = ? AND status ="
          " 'PENDING' ORDER BY id DESC LIMIT 1",
          (user_id,),
      ).fetchone()
      ...
      if ticket:
        ticket_id = ticket['id']
        db.execute(
            "UPDATE support_tickets SET status = 'COMPLETED', admin_reply = ?"
            ' WHERE id = ?',
      ...
      send_telegram(
          user_id,
          f'✅ <b>Support Ticket Resolved!</b>\n\n📩 <b>Admin'
          f' Reply:</b>\n{reply_text}',
      )
      send_telegram_voice(user_id, 'Your support ticket has been resolved.')
      send_telegram(
          ADMIN_ID,
          f'✅ Reply successfully sent to user ID <code>{user_id}</code>,'
          ' ticket status updated to <b>COMPLETED</b>.',
      )
  ...
  if update and 'callback_query' in update:
    ...
    if data.startswith('reply_ticket_'):
      ...
      send_telegram(
          chat_id,
          f'✍️ Please type your reply message to this user (ID:'
          f' <code>{user_id}</code>):',
      )
      return jsonify({'status': 'ok'})
    ...
    if data.startswith('approve_dep_'):
      ...
      if tx:
        ...
        requests.post(
            f'https://api.telegram.org/bot{BOT_TOKEN}/{edit_method}',
            json={
                'chat_id': chat_id,
                'message_id': message_id,
                content_key: (
                    f'✅ <b>DEPOSIT APPROVED & CONFIRMED</b>\nUser:'
                    f' <code>{user_id}</code>\nAmount: +${amount} USDT\nStatus:'
                    ' Balance Updated!'
                ),
                'parse_mode': 'HTML',
        ...
        send_telegram(
            user_id,
            f'🎉 <b>Deposit Approved!</b>\n\nYour deposit of <b>${amount} USDT</b>'
            ' has been credited to your balance successfully.',
        )
        send_telegram_voice(user_id, 'Your deposit has been approved.')
      db.close()
      ...
      if tx:
        ...
        send_telegram(
            user_id,
            f'❌ <b>Deposit Rejected</b>\n\nUnfortunately, your deposit request'
            f' of <b>${amount} USDT</b> was rejected.\nPlease check your TXID'
            ' or screenshot, or contact Support if you have any issues.',
        )
      db.close()
      ...
      if tx:
        ...
        requests.post(
            ...
            json={
                ...
                'message_id': message_id,
                content_key: (
                    f'✅ <b>WITHDRAWAL APPROVED & CONFIRMED</b>\nUser:'
                    f' <code>{user_id}</code>\nAmount: ${amount} USDT'
                ),
                'parse_mode': 'HTML',
                'reply_markup': {'inline_keyboard': []},
        ...
        send_telegram(
            user_id,
            f'🎉 <b>Withdrawal Approved!</b>\n\nYour withdrawal of <b>${amount}'
            ' USDT</b> has been successfully processed.',
        )
        send_telegram_voice(user_id, 'Your withdrawal has been approved.')
      db.close()
      ...
      if tx:
        ...
        send_telegram(
            user_id,
            f'⚠️ <b>Withdrawal Rejected</b>\n\nYour withdrawal request of'
            f' ${amount} USDT was rejected. The amount has been refunded to'
            ' your balance.',
        )
      db.close()
